...
# subset data to include only values of Vaccine coverage between 2009-2019
dt <- data[year_id %in% c('2009', '2019')]
...
dt[, percent_change := ((`2019`-`2009`)/`2009`)*100]
...
# Save file
print(paste('Saving:', outputFile5p)) 
pdf(outputFile5p, height=5.5, width=9)
...
for (i in 1:length(vaccines)) {
  g <- ggplot(dt[vaccine_name==vaccines[i]], aes(x = vaccine_name, y = percent_change)) +
    ...
    labs(title=paste('Change in vaccine coverage (with outliers labeled)'), y='Percent Change', x='Vaccine', 
         subtitle=paste0('between 2009 and 2019, among low-SDI countries')) + 
...
pdf(outputFile5q, height=9, width=11)
print(h)
dev.off()
...
# calculate median percent change for each vaccine in the 2009-2019 period
...
pdf(outputFile5r, height=9, width=11.5)
print(j)
dev.off()
...
# ##### ##### ##### #####
# # make trend graphs of countries that are missing values in 2009 as an extra check
# ##### ##### ##### #####
# dt5 <- copy(data)
# dt5 <- dt5[year_id==2009 & val==0]
# 
...
#          subtitle=paste('low-SDI locations that had 0% coverage in 2009')) + 
#     theme_minimal()+
#     ylim(0, 1)
# })
# 
