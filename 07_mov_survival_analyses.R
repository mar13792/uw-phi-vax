...
# Measles
################
dt1 <- dtnew[,.(total_with_card= .N), by = .(strata, assets)]

# # calculate how many children received mea1 according to health card only
dt2 <- dtnew[mea1_within_interval==1 | mea1_late==1,.(received_vaccine= .N), by = .(strata, assets)]

# calculate how many children did not receive the measles vaccine
dt3 <- dtnew[never_got_mea1==1 | early_mea1==1, .(no_vaccine=.N), by=.(strata, assets)]

# calculate how many children that were not vaccinated had a missed opportunity
dt4 <- dtnew[(never_got_mea1==1 | early_mea1==1) & mea1_missed_opportunity=="Yes", .(mop=.N), by=.(strata, assets)]
...
# calculate how many children has a vaccination card
dt1 <- dtnew2[,.(total_with_card= .N), by = .(strata, assets)]

# calculate how many children received all dpt vaccines
dt2 <- dtnew2[gotit==1,. (received_vaccine=.N), by=.(strata, assets)]

# calculate how many children did not receive all dpt doses
dt3 <- dtnew2[gotit==0,. (no_vaccine=.N), by=.(strata, assets)]

# calculate how many of the children that did not receive all dpt doses have a dpt missed opportunity
dt4 <- dtnew2[gotit==0 & dpt_missed_opportunity=="Yes",. (mop=.N), by=.(strata, assets)]
...
# calculate how many children have a vaccination card
dt1 <- dtnew3[,.(total_with_card= .N), by = .(strata, assets)]

# calculate how many children received the dpt1 dose
dt2 <- dtnew3[!is.na(age_at_dpt1),. (received_vaccine=.N), by=.(strata, assets)]

# calculate how many children did not receive dpt1 dose
dt3 <- dtnew3[is.na(age_at_dpt1),. (no_vaccine=.N), by=.(strata, assets)]

# calculate how many of the children that did not receive dpt1 dose have a dpt1 missed opportunity
dt4 <- dtnew3[is.na(age_at_dpt1) & dpt1_missed_opportunity==1,. (mop=.N), by=.(strata, assets)]
...
# # calculate how many children have a vaccination card
dt1 <- dtnew4[,.(total_with_card= .N), by = .(strata, assets)]

# # calculate how many children received dpt2
dt2 <- dtnew4[!is.na(age_at_dpt2),. (received_vaccine=.N), by=.(strata, assets)]

# calculate how many children did not receive dpt2 dose
dt3 <- dtnew4[is.na(age_at_dpt2),. (no_vaccine=.N), by=.(strata, assets)]

# calculate how many of the children that did not receive dpt2 dose have a dpt2 missed opportunity
dt4 <- dtnew4[is.na(age_at_dpt2) & dpt2_missed_opportunity==1,. (mop=.N), by=.(strata, assets)]
...
# # calculate how many children have a vaccination card
dt1 <- dtnew5[,.(total_with_card= .N), by = .(strata, assets)]

# # calculate how many children received dpt3
dt2 <- dtnew5[!is.na(age_at_dpt3),. (received_vaccine=.N), by=.(strata, assets)]

# calculate how many children did not receive dpt3 dose
dt3 <- dtnew5[is.na(age_at_dpt3),. (no_vaccine=.N), by=.(strata, assets)]

# calculate how many of the children that did not receive dpt3 dose have a dpt3 missed opportunity
dt4 <- dtnew5[is.na(age_at_dpt3) & dpt3_missed_opportunity==1,. (mop=.N), by=.(strata, assets)]
